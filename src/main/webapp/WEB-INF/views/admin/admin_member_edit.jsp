...
<div id="aMain_cont">
  ...
  <div id="aMd_wrap">
   ...
   <%--페이징에서 내가 본 쪽번호로 바로 이동하기 위한 책갈피 기능을 만들기 위해서 page네임피라미터 이름에 쪽번호를 담아서
   히든으로 전달한다. --%>
   
    <table id="aMd_t">
     <tr>
      <th class="left_box">회원아이디</th>
      <td><input name="mem_id" size="14" value="${am.mem_id}"
      readonly /></td>
     </tr>
     <tr>
     ...
     <tr>
      ...
      value="${am.mem_name}" /></td>
     </tr>
     <tr>
      <th class="left_box">우편번호</th>
      <td><input name="mem_zip" id="mem_zip" size="5"
      value="${am.mem_zip}" readonly />-<input name="mem_zip2"
      id="mem_zip2" size="5" value="${am.mem_zip2}" readonly />
      ...
      <td>
       <select name="mem_phone01">
        <c:forEach var="p" items="${phone}">
         <option value="${p}"
         <c:if test="${am.mem_phone01 == p}">
         ${'selected'}</c:if>>${p}</option>
        </c:forEach>
       </select>-<input name="mem_phone02" id="mem_phone02"
       size="4" maxlength="4" value="${am.mem_phone02}" />-
       <input name="mem_phone03" id="mem_phone03"
       size="4" maxlength="4" value="${am.mem_phone03}" />
      </td>
     </tr>
     <tr>
      ...
      value="${am.mail_id}" />@<input name="mail_domain"
      id="mail_domain" size="12" value="${am.mail_domain}"
      readonly />
      <select name="mail_list" onchange="domain_list();">
       <c:forEach var="mail" items="${email}">
        <option value="${mail}"
        <c:if test="${am.mail_domain == mail}">
