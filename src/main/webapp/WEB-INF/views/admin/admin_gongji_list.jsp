					...
					<c:forEach var="g" items="${glist}">
						<tr>
							...
							<td><a
								...
								<c:if test="${fn:length(g.gongji_title)>16}"> <%--공지제목이 16자를 초과하면 --%>
								 ${fn:substring(g.gongji_title,0,16)}...
								 <%--공지제목이 16자까지 출력되게 하고 나머지는 ...으로 표현 --%>
								</c:if>
								<c:if test="${fn:length(g.gongji_title) <= 16}">
									${g.gongji_title} 
								</c:if>									
								</a></td>
							<td align="center"><c:out value="${g.gongji_name}" /></td>
							<td align="center">${fn:substring(g.gongji_date,0,10)}</td>
							<%--0이상 10미만 사이의 년월일만 출력 --%>
...
취소를 클릭하면 false를 반환 --%></td>
    ...
    [다음]
			...
			<div id="bList_menu">
			    <button type="button" 
			    onclick="location='admin_gongji_write?page=${page}';" >공지작성</button>
				<c:if test="${(!empty find_field) && (!empty find_name)}">
